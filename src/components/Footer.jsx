    ...
    <footer className="bg-surface-container-high dark:bg-inverse-surface border-t-0 mt-section-gap">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-gutter py-section-gap px-margin-mobile max-w-container-max mx-auto flat no shadows">
        <div className="col-span-1 flex flex-col justify-center">
          <span className="font-headline-sm text-headline-sm text-secondary mb-4 uppercase">{siteConfig.businessName}</span>
          <p className="font-body-md text-body-md text-on-surface dark:text-inverse-on-surface opacity-80">
            © {new Date().getFullYear()} {siteConfig.businessName}. {siteConfig.tagline}.
          </p>
        </div>
        <div className="col-span-1 md:col-span-2 flex flex-wrap gap-8 justify-start md:justify-end items-center">
          ...
          {siteConfig.social.instagram && (
            <a href={siteConfig.social.instagram} className="font-label-caps text-label-caps text-on-surface-variant hover:text-secondary underline transition-all opacity-80 hover:opacity-100 uppercase tracking-widest">Instagram</a>
          )}
          {siteConfig.social.pinterest && (
            <a href={siteConfig.social.pinterest} className="font-label-caps text-label-caps text-on-surface-variant hover:text-secondary underline transition-all opacity-80 hover:opacity-100 uppercase tracking-widest">Pinterest</a>
          )}
          ...
          {siteConfig.social.youtube && (
            <a href={siteConfig.social.youtube} className="font-label-caps text-label-caps text-on-surface-variant hover:text-secondary underline transition-all opacity-80 hover:opacity-100 uppercase tracking-widest">YouTube</a>
          )}
        </div>
      </div>
    </footer>
