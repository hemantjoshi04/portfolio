...
export default function SEO({ title, description, image, url }) {
  ...
  const defaultTitle = settings.brandName || siteConfig.businessName;
  const tagline = settings.professionalTitle || siteConfig.tagline;
  const fullTitle = title ? `${title} | ${defaultTitle}` : `${defaultTitle} - ${tagline}`;

  // Default description if none provided
  const metaDescription = description || `Professional luxury makeup artistry by ${settings.brandName || siteConfig.businessName}. Specializing in bridal, editorial, and special occasion beauty.`;
